<script lang="ts">
  ...
  const cvContent: Record<'zh' | 'en', {
    ...
    experience: Entry[];
    honors: Honor[];
    labels: { education: string; experience: string; honors: string; profile: string; affiliation: string; location: string };
  }> = {
    zh: {
      labels: {
        education: '教育经历',
...
<style>
  ...
  .entry-list, .honor-list { border-top: 1px solid hsl(var(--border)); }
