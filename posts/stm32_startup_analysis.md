...
早期STM32启动文件采用**ARM汇编语法**（DCD、EXPORT等指令），主要面向Keil MDK-ARM工具链。同时ST也提供GCC/Clang兼容的版本，但维护成本较高。

**2022年：集中统一**

ST在GitHub的**CMSIS-Device**仓库进行了整合，将启动文件统一成**GNU汇编语法**（`.word`、`.global`等指令）：

- 单份代码既能被GCC/Clang使用，也能被Keil-AC6（新版armclang）兼容
- 减少维护分支，新芯片只需发布一套文件
- **影响范围**：仅限于`startup_stm32fxxx.s`和`system_stm32fxxx.c`
- **不涉及**：寄存器头文件、STD/HAL库源码（仍保持三重条件编译）

**现状**

| 开发工具 | 启动文件格式 | 获取方式 |
|---------|-----------|--------|
| Keil-AC5（armasm） | ARM语法 | 旧版Cube包或MDK自带 |
| Keil-AC6（armclang） | GNU语法 | GitHub最新版本 |
| GCC/Clang/CLion | GNU语法 | GitHub默认分支 |
